...
Para começar crie o repositório com nossos arquivos exportados de Coleção e Ambiente, e logo após clique em Actions: 
 
Na sequencia clique em fluxo simples, na opção configurar:
 
Criando o arquivo de Workflow (Fluxo de trabalho)
...
Agora criamos o trabalho que realmente executará a coleção POSTMAN.
...
Configurando a execução da coleção Postman no Wokflow do GitHub Actions
...
A etapa mostrada abaixo executa a coleção do Postman e cria o relatório html como resultado da execução do teste.
 

Fazendo upload de artefatos (relatórios, coleções e ambientes)
...
Ao clicar na execução (Update postman-pull-request.yml) vemos os detalhes da mesma:
 
Acima temos a execução do Newman e abaixo o relatório em HTML no formato zip.
Execução do Newman
